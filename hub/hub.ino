...
//     "forget" от пользователя - который ТОЛЬКО снимает флаг installed и убирает из
//     NVS, возвращая устройство в число обычных кандидатов (сам объект в рантайме НЕ
//     уничтожается, чтобы повторная установка того же устройства сразу показывала
//     уже известную конфигурацию - см. большой комментарий у DeviceManager::forget() в
//     DeviceManager.cpp).
...
// Единственный способ снять с устройства флаг установленного (для такого сам
// объект в DeviceManager СОХРАНЯЕТСЯ, чтобы повторная установка того же
// устройства сразу показывала уже известную конфигурацию, а не пустую до следующего
// планового MSG_CONFIG от узла) - для кандидата же реально освобождает слот раньше
// срабатывания LRU, как и раньше - см. большой комментарий у DeviceManager::forget() в
// DeviceManager.cpp.
...
void handleApiBeds() {
    ...
    for (int i = 0; i < MAX_BEDS; i++) {
        ...
        int foundIdx = deviceManager.find(b.mac);
        // Грядка может привязываться ТОЛЬКО к установленному модулю (см.
        // validateBedBinding() выше) - если найденное по MAC устройство сейчас
        // installed==false (его "забыли" через /api/forget - см. большой
        // комментарий у DeviceManager::forget() в DeviceManager.cpp, объект при этом мог
        // остаться в таблице как обычный кандидат, а не исчезнуть совсем), для
        // веб-страницы это ДОЛЖНО выглядеть как "модуль не найден", а не как
        // молчаливо всё ещё действующая привязка - иначе оператор не узнает, что модуль,
        // на который ссылается грядка, на самом деле уже не в списке установленных.
        int resolvedIdx = (foundIdx >= 0 && deviceManager.devices[foundIdx]->installed) ? foundIdx : -1;
        json += "{\"id\":" + String(i) +
...
void setup() {
 ...
 //   digitalWrite(AP_LED_PIN, LOW); // безопасный начальный дефолт - реальное состояние выставит setApEnabled(), когда оператор явно включит AP
    ...
    if (incomingPacketQueue == nullptr) {
        ...
        radioReady = initEspNow(); // см. initEspNow() выше
    }
