...
// GET /api/status - служебная информация Хаба, сейчас про часы и точку
// доступа - веб-страница опрашивает это в том же цикле автообновления,
// что и /api/devices, чтобы показать оператору визуальное подтверждение,
// что синхронизация времени сработала.
