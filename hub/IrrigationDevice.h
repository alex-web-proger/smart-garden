...
struct ValveSchedule {
    ...
    uint16_t durationSec = 0;  // продолжительность полива, в СЕКУНДАХ - ИМЕЕТ СМЫСЛ ТОЛЬКО при mode==1
                                ...
                                // клапана (см. IrrigationCommand.duration_sec в GardenProtocol.h) - и, так же,
                                // как и duration_sec там, ТЕПЕРЬ РЕАЛЬНО ОГРАНИЧИВАЕТ реальное открытие
                                // клапана на самом узле - узел (mode!=2 в IrrigationCommand) сам закроется по
                                // истечении этого времени (см. valveCloseAtMs/checkDurationTimeouts() в
                                // flow_node.ino) - НО САМ ПО РАСПИСАНИЮ (т.е. периодическая автоматическая
                                // отправка MSG_COMMAND без участия оператора, по intervalDays) всё ещё НЕ
                                // РЕАЛИЗОВАН - это значение только сохраняется и подставляется в
                                // отправляемую команду, когда оператор вручную нажимает кнопку полива
                                // грядки (см. toggleBedIrrigation() в WebPage.h).
