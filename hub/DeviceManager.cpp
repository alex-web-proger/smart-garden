...
bool DeviceManager::forget(int idx) {
    ...
    if (devices[idx]->installed) {
        // УЖЕ УСТАНОВЛЕННОЕ устройство - "забыть" его значит снять флаг
        // installed и убрать из следующей записи в NVS (saveToNVS() и так пишет
        // только installed==true), НО НЕ уничтожать сам объект в рантайме (то
        // есть НЕ destroyAt(idx), как было раньше) - тот же физический узел, скорее
        // всего, продолжает слать трафик как обычно, и вся уже накопленная о нём
        // информация (valveCount/mode/hasFlowSensor из последнего принятого MSG_CONFIG,
        // накопленный объём воды и т.п.) остаётся ДОСТОВЕРНОЙ - она не привязана к
        // факту "установлено", а только к факту "узел когда-то прислал MSG_CONFIG".
        //
        // РАНЬШЕ ЗДЕСЬ ЗВАЛСЯ destroyAt(idx), и это БЫЛА ОШИБКОЙ: повторная установка
        // ТОГО ЖЕ устройства СРАЗУ после forget показывала пустую конфигурацию
        // (valveCount=0) на веб-странице, пока узел сам не пришлёт очередной плановый
        // MSG_CONFIG (интервал около часа, см. CONFIG_INTERVAL_MS в flow_node.ino) - ПРОТОКОЛ
        // НЕ предусматривает способа попросить у узла конфигурацию вне плана (единственный
        // способ заставить узел сразу переслать MSG_CONFIG - отправить ему MSG_SET_CONFIG,
        // что само по себе меняет его настройки - цена слишком высока ради простого
        // обновления отображения). Перезагрузка САМОГО ХАБА "чинила" это лишь СЛУЧАЙНО:
        // сброшенный в 0 packet_id Самого Хаба заставлял GardenNode::handleIncoming() на узле
        // ошибочно распознать очередной announce как "Хаб перезагрузился" и тут же прислать
        // свою конфигурацию заново (см. большой комментарий там про определение перезагрузки
        // по разрыву packet_id) - побочный эффект, а не надёжный механизм, никак не связанный с
        // самим forget()/install() здесь. Оставляя объект как есть, просто снимая installed -
        // устройство становится ОБЫЧНЫМ кандидатом (участвует в LRU-вытеснении наравне с
        // остальными, см. findOrRegister()) - при повторной установке уже накопленная
        // конфигурация видна сразу же, без ожидания и/или перезагрузки Чего-либо.
        devices[idx]->installed = false;
        saveToNVS();
        return true;
    }

    // КАНДИДАТ (никогда не был установлен, либо это повторный forget() после
    // "мягкого" снятия installed выше) - здесь "забыть" означает буквально убрать из
    // списка/освободить слот - именно этого ожидает оператор, нажимая "Забыть" на
    // КАНДИДАТЕ (см. handleApiForget() в hub.ino) - в отличие от уже установленного
    // устройства выше, у кандидата нет накопленной ценной конфигурации, которую стоило бы
    // сохранять, и слот стоит освободить сразу, а не ждать LRU.
    destroyAt(idx);
    saveToNVS();
    return true;
}
