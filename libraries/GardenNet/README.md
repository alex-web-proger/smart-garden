...
входящих пакетов, сброс dedup по announce Хаба о (пере)загрузке, тот же
аnnounce попутно несёт текущее время Хаба - узел автоматически выставляет по
нему свои часы (`isTimeSynced()`/`currentTimeString()`, подробнее - `PROTOCOL.md`
§12.6), случайный джиттер интервалов `MSG_CONFIG`/`MSG_TELEMETRY`, watchdog по
